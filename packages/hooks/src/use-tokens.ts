...
export const useTokens = ({ chainId, defaultAddresses }: TokensProps) => {
  ...
  const fetchToken = useCallback(
    async (address: string) => {
      try {
        const res = await fetch(`${TOKEN_API}?pageSize=100&page=1&query=${address}&chainIds=${chainId}`);
  ...
  const fetchTokenList = useCallback(() => {
    Promise.all([
      ...
      ...(defaultAddresses ? defaultAddresses.split(',').map(address => fetchToken(address)) : []),
    ]).then(results => {
