...
export function TargetPoolState({
  ...
  const isMaxTick =
    pool !== 'loading' && isUniV3 && tickUpper === nearestUsableTick(MAX_TICK, (pool as UniV3Pool).tickSpacing);
  ...
  const poolTick = !isUniV3
    ...
    : pool === 'loading'
      ...
      : (pool as UniV3Pool).tick % (pool as UniV3Pool).tickSpacing === 0
        ...
        : nearestUsableTick((pool as UniV3Pool).tick, (pool as UniV3Pool).tickSpacing);

  const increaseTickLower = () => {
    if (!isUniV3) return;
    if (pool === 'loading' || poolTick === undefined) return;
    const newTick =
      tickLower !== null ? tickLower + (pool as UniV3Pool).tickSpacing : poolTick + (pool as UniV3Pool).tickSpacing;
  ...
  const increaseTickUpper = () => {
    ...
    if (pool === 'loading' || poolTick === undefined) return;
    const newTick =
      tickUpper !== null ? tickUpper + (pool as UniV3Pool).tickSpacing : poolTick + (pool as UniV3Pool).tickSpacing;
  ...
  const decreaseTickLower = () => {
    if (!isUniV3) return;
    if (pool === 'loading' || poolTick === undefined) return;
    const newTick = (tickLower !== null ? tickLower : (pool as UniV3Pool).tick) - (pool as UniV3Pool).tickSpacing;
  ...
  const decreaseTickUpper = () => {
    if (!isUniV3) return;
    if (pool === 'loading' || poolTick === undefined) return;
    const newTick = (tickUpper !== null ? tickUpper : poolTick) - (pool as UniV3Pool).tickSpacing;
  ...
  useEffect(() => {
    if (pools === 'loading' || !initialTick || !isUniV3 || !('tickSpacing' in pools[1]) || tickLower || tickUpper)
      return;
    if (initialTick.tickLower % pools[1].tickSpacing === 0 && initialTick.tickUpper % pools[1].tickSpacing === 0) {
      setTickLower(initialTick.tickLower);
      setTickUpper(initialTick.tickUpper);
    }
  }, [initialTick, isUniV3, pools, setTickLower, setTickUpper, tickLower, tickUpper]);
          ...
          <>
            <div className="flex items-center gap-1">
              ...
              <div className="text-text">
                1 {revertDisplay ? pool.token1.symbol : pool.token0.symbol} = {poolPrice}{' '}
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      priceRange === FULL_PRICE_RANGE ? 'w-max-content' : 'flex-1',
                      selectedRange?.range === priceRange ? 'border-primary text-primary' : '',
                    )}
                    onClick={() => handleSelectPriceRange(priceRange as typeof FULL_PRICE_RANGE | number)}
                  >
                    {priceRange === FULL_PRICE_RANGE ? priceRange : `${Number(priceRange) * 100}%`}
                  </button>
                );
              })}
            </div>
                      ...
                      if (tick !== undefined) {
                        const t =
                          tick % (pool as any).tickSpacing === 0
                            ? tick
                            : nearestUsableTick(tick, (pool as any).tickSpacing);
                      ...
                      if (tick !== undefined) {
                        const t =
                          tick % (pool as UniV3Pool).tickSpacing === 0
                            ? tick
                            : nearestUsableTick(tick, (pool as UniV3Pool).tickSpacing);
