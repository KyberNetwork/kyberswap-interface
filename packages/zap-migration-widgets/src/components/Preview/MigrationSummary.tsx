      ...
      <AccordionItem value="item-1">
        ...
        <AccordionContent className="px-4 pb-4 pt-0 border border-stroke !border-t-0 rounded-b-md">
          ...
          <div className="flex items-start text-subText gap-2 mt-3">
            ...
            <div className="flex-1 text-xs">
              <Trans>
                ...
                {sourcePool.token1.symbol} from {sourceDexName as string}
              </Trans>
            </div>
          </div>
          ...
          <div className="flex items-start text-subText gap-2 mt-3">
            ...
            <div className="flex-1 text-xs">
              {swapActions.map((item, index) => {
                const poolElement = item.poolAddress ? (
                  <a
          ...
          <div className="flex items-start text-subText gap-2 mt-3">
            ...
            <div className="flex-1 text-xs">
              <Trans>
                Add {formatTokenAmount(addedLiquidity.addedAmount0, targetPool.token0.decimals, 8)}{' '}
                {targetPool.token0.symbol} and{' '}
                {formatTokenAmount(addedLiquidity.addedAmount1, targetPool.token1.decimals, 8)}{' '}
                {targetPool.token1.symbol} into {targetDexName as string} in the selected fee pool
              </Trans>
            </div>
          </div>
        </AccordionContent>
