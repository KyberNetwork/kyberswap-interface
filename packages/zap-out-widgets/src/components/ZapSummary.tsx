...
export function ZapSummary() {
  ...
  const swapAction = route?.zapDetails.actions.find(item => item.type === 'ACTION_TYPE_AGGREGATOR_SWAP') as
    | AggregatorSwapAction
    | undefined;

  const amountIns: { token: Token; amount: bigint }[] = [];
  swapAction?.aggregatorSwap?.swaps.forEach(item => {
    const token = poolTokens.find(pt => pt.address.toLowerCase() === item.tokenIn.address.toLowerCase());
        ...
        <AccordionContent className="px-4 pb-4 pt-0 border border-stroke !border-t-0 rounded-b-md">
          ...
          <div className="flex gap-2 mt-2">
            ...
            <div className="flex-1 text-subText text-xs">
              Remove {amountToken0 !== 0n ? formatTokenAmount(amountToken0, token0?.decimals || 18) : ''}{' '}
              {token0?.symbol}
              {amountToken1 !== 0n
              ...
              {feeAmount0 !== 0n || feeAmount1 !== 0n ? (
                <>
                  and claim fee {feeAmount0 !== 0n ? formatTokenAmount(feeAmount0, feeToken0?.decimals || 18) : ''}{' '}
          ...
          <div className="flex gap-2 mt-3 items-center">
            ...
            <div className="text-xs text-subText">
              Receive {formatTokenAmount(amountOut, tokenOut?.decimals || 18)} {tokenOut?.symbol}
            </div>
          </div>
        </AccordionContent>
      </AccordionItem>
