...
import { CHAIN_ID_TO_CHAIN, DEXES_INFO, NATIVE_TOKEN_ADDRESS, NETWORKS_INFO, PATHS } from '@/constants';
import {
  AddLiquidityAction,
...
import { PI_LEVEL, formatCurrency, formatNumber, formatWei, friendlyError, getPriceImpact } from '@/utils';
...
export default function Preview({
  ...
  const { source, revertPrice: revert, toggleRevertPrice, tokensIn, amountsIn, tokensInUsdPrice } = useZapState();
  ...
  useEffect(() => {
    fetch(`${PATHS.ZAP_API}/${CHAIN_ID_TO_CHAIN[chainId]}/api/v1/in/route/build`, {
      method: 'POST',
      body: JSON.stringify({
        sender: account,
        recipient: account,
          ...
          <div className="text-[14px] flex gap-4">
            <div className="flex flex-col gap-1">
              <div className="flex gap-1">
                ...
                <div>
                  {formatDisplayNumber(positionId !== undefined ? amount0 : +addedAmount0, { significantDigits: 4 })}{' '}
                  {pool?.token0.symbol}
                </div>
              </div>
            ...
            <div className="flex flex-col gap-1">
              <div className="flex gap-1">
                ...
                <div>
                  {formatDisplayNumber(positionId !== undefined ? amount1 : +addedAmount1, { significantDigits: 4 })}{' '}
                  {pool?.token1.symbol}
                </div>
              </div>
              {positionId && (
