...
export const uint256ToFraction = (
  value: string,
  decimals = RESERVE_USD_DECIMALS
) =>
  new Fraction(
    value,
...
const parseString = (value: string): Fraction => {
  try {
    ...
    const [negative, _currency, integer, decimal, exponentUnit, e, _unit] =
      parseNumPart(value);
    const trimedNumerator = (negative + integer.replace(/,/g, "") + decimal)
      .replace(/^0+/, "")
      .replace(/^-0+/, "-");
    const exponent =
      Number(e || "0") +
      Number(
        exponentUnit
          ? unitMapping[exponentUnit.toLowerCase().trim()] ?? "0"
          : "0"
      ) -
      decimal.length;
    ...
    if (
      ...
      const valueStr = (() => {
        ...
        if (value instanceof CurrencyAmount)
          return value.toFixed(value.currency.decimals);
      ...
      return new Fraction(
        valueStr.replace(".", ""),
        "1" + "0".repeat(valueStr.split(".")[1]?.length || 0)
      );
    }
...
export const formatDisplayNumber = (
  ...
  if (!allowDisplayNegative && parsedFraction.lessThan(BIG_INT_ZERO))
    return fallbackResult;
  if (!allowDisplayZero && parsedFraction.equalTo(BIG_INT_ZERO))
    return fallbackResult;

  const shownFraction =
    style === "percent" ? parsedFraction.multiply(100) : parsedFraction;
  const absShownFraction = shownFraction.lessThan(0)
    ? shownFraction.multiply(-1)
    : shownFraction;

  if (
    absShownFraction.lessThan(BIG_INT_ONE) &&
  ...
  const formatter = Intl.NumberFormat("en-US", {
    ...
    // roundingPriority:
    //   fractionDigits && significantDigits ? "lessPrecision" : undefined,
  });
