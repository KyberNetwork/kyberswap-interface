...
export const RowWrapper = ({ children, className, ...rest }: HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      'grid grid-cols-[44px_minmax(0,1.2fr)_minmax(0,1.2fr)_minmax(0,1fr)_minmax(0,1.4fr)_48px_88px] items-center gap-2 text-sm max-sm:grid-cols-[48px_minmax(0,1fr)_minmax(0,0.8fr)] max-sm:gap-y-2',
      className,
    )}
    {...rest}
  >
...
const TableHeader = () => {
  return (
    <RowWrapper className="cursor-default bg-background px-4 py-3 text-xs font-medium uppercase tracking-[0.04em] text-subText">
      <span className="max-sm:row-span-2 max-sm:self-center">CHAIN</span>
