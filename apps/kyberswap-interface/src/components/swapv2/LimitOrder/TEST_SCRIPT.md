...
- Expected result: submit button shows `Exceeds available amount`.
...
### 27. My Orders invalidation after create
- Example: create order `100 USDT -> 0.04 ETH`.
- Expected invalidated tags:
  - `GET_LIMIT_ORDER_LIST`
  - `GET_LIMIT_ORDER_INSUFFICIENT`
  - `GET_LIMIT_ORDER_ACTIVE_MAKING_AMOUNT`

### 28. My Orders invalidation after take/fill
- Example: fill order requiring `100 USDT`.
- Expected invalidated tags:
  - `GET_LIMIT_ORDER_LIST`
  - `GET_LIMIT_ORDER_INSUFFICIENT`
  - `GET_LIMIT_ORDER_ACTIVE_MAKING_AMOUNT`

### 29. My Orders invalidation after cancel
...
### 30. Order Book independence
- Example: create, fill, or cancel any order.
- Expected result: action success does not invalidate `GET_LIMIT_ORDER_BOOK`.
- Expected result: Order Book still updates through its own polling, focus refetch, and manual refresh.

### 31. Active making amount refresh after state-changing action
- Example: user has active reserved amount `300 USDT`, then creates/cancels/fills one active order.
- Expected result: `GET_LIMIT_ORDER_ACTIVE_MAKING_AMOUNT` is invalidated.
- Expected result: create form notice and allowance calculation update without waiting for a full page refresh.

## URL / Tab Behavior

### 32. Reserved order notice link
