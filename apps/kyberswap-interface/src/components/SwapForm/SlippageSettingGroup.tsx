...
export default function SlippageSettingGroup({
  ...
  if (chainId === ChainId.ZKSYNC && !isPartnerSwap) {
    rightButton = (
      <div className="flex w-fit cursor-pointer items-center" role="button" onClick={onOpenGasToken}>
        ...
        <span className="ml-1.5 font-medium text-text">{paymentToken ? paymentToken.symbol : 'ETH'}</span>
        <DropdownSVG />
      </div>
    )
  }
