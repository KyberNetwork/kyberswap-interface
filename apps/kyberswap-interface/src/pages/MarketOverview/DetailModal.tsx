...
export default function DetailModal({
  ...
  return (
    <Modal isOpen={!!tokenToShow} onDismiss={onDismiss} width="100%" maxWidth="600px">
      {tokenToShow ? (
        <Stack className="w-full gap-5 p-6">
          <HStack className="justify-between gap-1">
            <HStack className="flex-1 items-center gap-1.5">
              <img
          ...
          <Stack className="gap-6 max-sm:gap-2">
            <div
              className={cn(
                'grid h-fit rounded-2xl bg-white-08 px-4 py-3',
            ...
            >
              ...
              <Stack className="gap-1">
                ...
                <span>{formatDisplayNumber(tokenToShow.allTimeLow, { style: 'currency', fractionDigits: 2 })}</span>
              </Stack>
              <Stack className="gap-1">
                <span className="text-xs text-subText">
                  <Trans>All Time High</Trans>
                </span>
                <span>{formatDisplayNumber(tokenToShow.allTimeHigh, { style: 'currency', fractionDigits: 2 })}</span>
              </Stack>
            </div>

            <Stack className="gap-2">
              {!upToSmall ? (
                <div className={cn('grid', rowGridClass)}>
                  <div />
                  <span className="text-right text-xs font-medium text-subText">
                ...
                <HStack className="items-center justify-end gap-3">
                  ...
                  <HStack className="w-fit rounded-full border border-border bg-buttonBlack p-px">
                    <TabItem
              ...
              <Stack className="gap-4">
                {tokenToShow.tokens
                  ...
                  .map(token => {
                    ...
                    return (
                      <div key={token.chainId} className={cn('grid items-center', rowGridClass)}>
                        <HStack className="items-center gap-3">
                          ...
                          <Stack className="flex-1 gap-0.5 overflow-hidden">
                            <HStack className="items-end text-base">
                              {tokenToShow.symbol}
                              {quoteSymbol && <span className="truncate text-sm text-subText">/{quoteSymbol}</span>}
                            </HStack>
                            <HStack className="text-xs text-subText">
