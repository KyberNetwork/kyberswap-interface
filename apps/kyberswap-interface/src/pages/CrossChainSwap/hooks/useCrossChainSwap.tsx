...
export const CrossChainSwapRegistryProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const getQuote = useCallback(async () => {
    ...
    const r: TokenPricesResponse = await fetch(`${TOKEN_API_URL}/v1/public/tokens/prices`, {
      method: 'POST',
    ...
    const tokenInUsd = isEvmCurrency(currencyIn)
      ? getTokenMidPriceUsd(r.data?.[fromChainId]?.[currencyIn.wrapped.address])
      : 0
    const tokenOutUsd = isEvmCurrency(currencyOut)
      ? getTokenMidPriceUsd(r.data?.[toChainId]?.[currencyOut.wrapped.address])
      : 0
    ...
    const sendSolanaTransaction = connection.sendTransaction as unknown as Parameters<typeof adaptSolanaWallet>[3]
