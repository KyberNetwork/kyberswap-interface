  ...
  const getQuote = useCallback(async () => {
    ...
    const getQuotesWithCancellation = async (params: QuoteParams | NearQuoteParams) => {
      ...
      if (isSameChainEvmSwap) {
        ...
        const isKyberSwapSupported = kyberswapAdapter?.canSupport(category) ?? true
      ...
      // For cross-chain swaps, use the streaming API (new logic)
      ...
      // Filter adapters based on both excludedSources and canSupport(category)
      const supportedAdapters = allAdapters.filter(adapter => adapter.canSupport(category))
      ...
      const excludedSourceNames = allAdapters
        .filter(adapter => excludedSources.includes(adapter.getName()) || !adapter.canSupport(category))
        .map(adapter => adapter.getName())
      ...
      console.log('Using streaming API for cross-chain swap:', apiUrl)
