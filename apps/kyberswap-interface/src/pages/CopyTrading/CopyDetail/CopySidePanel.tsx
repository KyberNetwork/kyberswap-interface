...
import { formatUsd, percent, signedUsd } from 'pages/CopyTrading/helpers'
...
const Card = ({ bodyClassName, children, title }: CardProps) => (
...
const CopyStatusCard = ({ run }: CopyStatusCardProps) => {
  const isClosed = run.status === 'closed'
  const realizedPnl = signedUsd(run.realizedPnlUsd)

  if (isClosed) {
    return (
      <Card title="Closed Copy">
        <HStack className="items-center justify-between">
          <span className="text-subText">Capital In</span>
          <span className="text-lg font-medium text-text">{formatUsd(run.capitalInUsd)}</span>
        </HStack>
        <HStack className="items-center justify-between">
          <span className="text-subText">Capital Out</span>
          <span className="text-lg font-medium text-text">{formatUsd(run.capitalOutUsd)}</span>
        </HStack>
        <HStack className="items-center justify-between">
          <span className="text-subText">Realised P&L</span>
          <span className={cn('text-xl font-medium', realizedPnl.startsWith('-') ? 'text-red' : 'text-primary')}>
            {realizedPnl}
          </span>
        </HStack>
  ...
  return (
    <Card title="Your Current Copy">
      <HStack className="items-center justify-between">
        <span className="text-subText">Capital In</span>
        <span className="text-xl font-medium text-primary">{formatUsd(run.capitalInUsd)}</span>
      </HStack>
      <HStack className="items-center justify-between">
        <span className="text-subText">Portfolio Value</span>
        <span className="text-lg font-medium text-text">{formatUsd(run.portfolioValueUsd)}</span>
      </HStack>
      <HStack className="items-center justify-between">
        <span className="text-subText">Unrealised P&L</span>
        <span
          className={cn('text-lg font-medium', Number(run.unrealizedPnlUsd || 0) < 0 ? 'text-red' : 'text-primary')}
        >
          {signedUsd(run.unrealizedPnlUsd)}
        </span>
      </HStack>
