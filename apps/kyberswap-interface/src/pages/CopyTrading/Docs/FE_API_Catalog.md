...
current valuation, lifecycle, and unrealized P&L. Route output quantities are
deliberately not exposed. If the selectable position set changes, discard the
old preparation and prepare again.
...
upfront fee, ratio, unresolved count, and cashback. It intentionally excludes
private route output and effective-slippage fields.
...
6. Require `valueRaw === "0"`.
7. Reject a response past `reprepareAfter` or
   `liquidationConfigDeadline`.
8. Submit `to`, `data`, and `valueRaw` unchanged.
