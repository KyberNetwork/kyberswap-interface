...
export const VaultCard = ({ className, ...rest }: HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      'relative flex flex-col gap-2 rounded-xl bg-[rgba(54,39,86,0.2)] p-4 transition-colors duration-200',
...
type VaultDepositButtonProps = ButtonHTMLAttributes<HTMLButtonElement> & { $disabled?: boolean }
export const VaultDepositButton = ({ $disabled, className, ...rest }: VaultDepositButtonProps) => (
  <button
    type="button"
    className={cn(
      'relative z-[1] flex items-center justify-center rounded-xl border border-solid border-primary px-3 py-1',
