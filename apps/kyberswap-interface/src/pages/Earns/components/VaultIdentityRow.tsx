...
import { toDisplayLogo, toDisplaySymbol } from 'pages/Earns/utils/vault'
...
const VaultIdentityRow = ({ vault, className }: { vault: VaultApiDetailItem; className?: string }) => {
  const symbol = toDisplaySymbol(vault.baseToken, vault.underlyingToken)

  return (
    <div className={cn('flex w-full items-center gap-2', className)}>
      <span className="relative flex shrink-0 items-center">
        <TokenLogo src={toDisplayLogo(vault.baseToken, vault.underlyingToken)} alt={symbol} size={20} />
        {vault.chain?.logo ? (
          <TokenLogo
      ...
      <span className="text-base leading-6 text-white">
        {symbol} <span className="text-subText">{t`Yield`}</span>
      </span>
      {vault.provider?.name ? (
