...
import { calcPercentFilledOrder, formatRateOrder, uint256ToFraction } from './helpers'
...
const Rate = ({
  ...
  if (order) {
    ...
    rateStr = invertRate ? rateInfo.invertRate : rateInfo.rate
  }
  return (
    <Value
  ...
  const confirmationContent = useCallback(() => {
    return (
      <Flex flexDirection={'column'} width="100%">
        ...
        {/* {isEdit && (
          <Flex>
            <div>Cancel {step === LimitOrderActions.CANCEL ? '(x)' : null}</div>
            <div>Create {step === LimitOrderActions.CREATE ? '(x)' : null}</div>
          </Flex>
        )} */}
      </Flex>
    )
...
export function CancelOrderModal({
  ...
  const { tradeInfo } = useBaseTradeInfo(currencyIn, currencyOut) // todo check api call
  const confirmationContent = useCallback(
    () =>
      flowState.errorMessage ? (
        <TransactionErrorContent onDismiss={onDismiss} message={flowState.errorMessage} />
