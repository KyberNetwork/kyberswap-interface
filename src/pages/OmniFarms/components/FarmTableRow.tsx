...
const ChainLogo = styled.div<{ src: string }>(({ src }) => ({
...
const getFeeOrAMPTag = (pool: NormalizedFarm) => {
  ...
  if ('feeTier' in pool.pool) return `Fee ${+pool.pool.feeTier / ELASTIC_BASE_FEE_UNIT}%`
...
export default function FarmTableRow({ farm }: { farm: NormalizedFarm }) {
  ...
  const isAllRangesInactive = false
  const isApproved = true
  const canUnstake = true
  const canUpdateLiquidity = true
  ...
  return (
    <Row>
      ...
      <Text textAlign="right">{formatDisplayNumber(farm.stakedTvl, { style: 'currency', significantDigits: 6 })}</Text>
