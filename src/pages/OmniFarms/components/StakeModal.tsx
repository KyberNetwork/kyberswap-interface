...
export default function StakeModal({ farm, onDismiss }: { farm: NormalizedFarm; onDismiss: () => void }) {
  ...
  const newPositions = useMemo(() => farm.availablePositions.filter(pos => !pos.depositedPosition), [farm])
  const depositedPositions = useMemo(() => farm.availablePositions.filter(pos => !!pos.depositedPosition), [farm])

  const [tab, setTab] = useState<'new' | 'deposited'>(() => (newPositions.length ? 'new' : 'deposited'))

  const availablePositions = useMemo(
    () => (tab == 'new' ? newPositions : depositedPositions),
    [tab, newPositions, depositedPositions],
  )
      ...
      <Flex width="100%" padding="20px" flexDirection="column" sx={{ gap: '20px' }} backgroundColor={theme.background}>
        ...
        <Text color={theme.subText} fontSize="12px" lineHeight="16px">
          <Trans>
            ...
            in-range position will earn rewards. Read more <ExternalLink href="/TODO">here ↗</ExternalLink>
          </Trans>
        </Text>

        <Flex justifyContent="space-between" alignItems="center" fontSize="14px">
          <FilterGroup>
            {!!newPositions.length && !!depositedPositions.length && (
              <>
                <FilterItem active={tab === 'new'} onClick={() => setTab('new')}>
                  <Trans>New Positions</Trans>
                </FilterItem>

                <FilterItem active={tab === 'deposited'} onClick={() => setTab('deposited')}>
            ...
            <ButtonPrimary
              ...
              onClick={async () => {
                try {
                  ...
                  let hash = ''
                  if (tab === 'new') {
                    const estimateGas = await contract.estimateGas.depositAndJoin(farm.pid, selectedNfts)
                    const tx = await contract.depositAndJoin(farm.pid, selectedNfts, {
                    ...
                    const liqs = selectedNfts.map(id => {
                      ...
                      return BigNumber.from(tmp?.liquidity || '0')
                        .sub(tmp?.stakedLiq || '0')
                        .toString()
                    })
                    const estimateGas = await contract.estimateGas.join(farm.pid, selectedNfts, liqs)
                    const tx = await contract.join(farm.pid, selectedNfts, liqs, {
                      gasLimit: calculateGasMargin(estimateGas),
                    })
                    hash = tx.hash
                  }
