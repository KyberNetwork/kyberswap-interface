...
const Chart = ({
  ...
  const chartData = rawChartData.chart_info.map(item => ({
    time: item.timestamp * 1000,
    value:
      chartCategory === TrueSightChartCategory.TRADING_VOLUME ? item.trading_volume.toString() : item.price.toString(),
  }))
  ...
  // )
  const mainValue = hoverValue
    ? formattedNum(hoverValue.toString(), true)
    : chartData.length
    ? formattedNum(chartData[chartData.length - 1].value, true)
    : '--'
  const subValue = `-$999999999 (-99%)`
