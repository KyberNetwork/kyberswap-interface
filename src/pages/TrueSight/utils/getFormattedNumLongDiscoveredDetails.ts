  ...
  return {
    ...
    pricePercent:
      price <= 0 || price_discovered <= 0
        ? '--'
        : formattedNumLong((price_discovered / price) * 100 - 100, false) + '%',
    ...
    tradingVolumePercent:
      trading_volume <= 0 || trading_volume_discovered <= 0
        ? '--'
        : formattedNumLong((trading_volume_discovered / trading_volume) * 100 - 100, false) + '%',
    marketCap: market_cap_discovered <= 0 ? '--' : formattedNumLong(market_cap_discovered, true),
    marketCapPercent:
      market_cap <= 0 || market_cap_discovered <= 0
        ? '--'
        : formattedNumLong((market_cap_discovered / market_cap) * 100 - 100, false) + '%',
  }
}
