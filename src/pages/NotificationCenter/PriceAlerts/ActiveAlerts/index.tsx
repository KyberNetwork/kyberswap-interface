...
import { useDeleteAllAlertsMutation, useGetAlertStatsQuery, useGetListAlertsQuery } from 'services/priceAlert'
  ...
  const renderDeleteAllButton = () => {
    return (
      <DeleteAllAlertsButton
        isLoading={isLoading || deleteAllActiveResult.isLoading}
        disabled={!data?.pagination?.totalItems || !account}
        onClick={async () => {
          if (!account) {
