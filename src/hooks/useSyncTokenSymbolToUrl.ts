...
export default function useSyncTokenSymbolToUrl(
  ...
  const isSamePath = pathname.startsWith(path) // todo refactor, change network có vẻ sai, url chớp chớp ở trang limit
  ...
  useEffect(() => {
    if (refChain.current && Object.values(defaultTokens)[0]?.chainId === refChain.current) {
      ...
      setTimeout(() => {
        findTokenPairFromUrl(chainId) // todo
      })
  ...
  // todo sometimes token import bấm 2 lần mới lên: type and swap
}
