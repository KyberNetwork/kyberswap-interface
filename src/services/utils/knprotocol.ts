import { ChainId, Fraction, Token as TokenSDK, WETH } from '@kyberswap/ks-sdk-core'
...
type KNResponseType<T> = {
  data: T & { pagination: { totalRecords: number } }
}
export type KNToken = {
  id: string
  symbol: string
...
const transformElasticPool = (pool: AllChainElasticPool, timeframe: '24h' | '7d' | '30d'): ElasticPoolDetail => {
  const chainId = SUPPORTED_NETWORKS.find(chain => NETWORKS_INFO[chain].poolFarmRoute === pool.chain) || ChainId.MAINNET
  return {
    chainId,
    ...
    feeUSDLast:
      Number(pool.feesUsd) -
      (timeframe === '24h'
        ? Number(pool.feesUsdOneDayAgo)
        : timeframe === '7d'
        ? Number(pool.feesUsd7DaysAgo)
        : timeframe === '30d'
        ? Number(pool.feesUsd30DaysAgo)
        : Number(pool.feesUsdOneDayAgo)),
    volumeUSDLast:
    ...
    allPositionsUSD: pool.positions?.reduce((acc, cur) => acc.add(parseFraction(cur.amountUSD)), new Fraction(0)),
  }
}

const transformClassicPool = (pool: AllChainClassicPool, timeframe: '24h' | '7d' | '30d'): ClassicPoolData => {
...
export function transformResponseAllChainAllPool(
  ...
  try {
    return {
      pools: response.data.pools?.reduce((acc, pool) => {
